...
test.edit_row_table("Type",{"COUNT":2,"MSG":"NULL"},"TypeID",0) # UPDATE type SET COUNT='2', MSG="NULL" WHERE TypeID="0" (need have in table some Type Error where ID is 0)
print(test.get_rows_ID("Type","COUNT",0,3)) # SELECT TypeID FROM Tpe WHERE COUNT BETWEEN '0' AND '3'
try:
    test.remove_row_via_ID("Errors","ErrorID",test.get_rows_ID("Errors","First","NULL")) # the part below must work first
except Exception as error:
    print("Error: "+ str(error)+"the part \"Not Working#1\" must work first")
# Not Working
test.add_Error(language_identity.identify(bug_search.get_error_from_log("""Traceback (most recent call last):
